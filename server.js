...
function createComment(url, request) {
  ...
  if (requestComment
    ...
    database.comments[database.nextCommentId] = comment;
...
function updateComment(url, request) {
  ...
  if (requestComment && database.comments[id] && request.body.comment.body) {
    database.comments[id].body = request.body.comment.body;
    response.status = 200;
  } else if (request.body === undefined || request.body.comment === undefined) {
    response.status = 400;
  } else {
    response.status = 404;
  }
...
function deleteComment(url) {
  ...
  if (database.comments[id]) {
    let username = database.comments[id].username;
    let articleId = database.comments[id].articleId;

    database.users[username].commentIds = database.users[username].commentIds.filter(commentId => commentId != id);
    database.articles[articleId].commentIds = database.articles[articleId].commentIds.filter(commentId => commentId != id);
...
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let upvotedComment = database.comments[id];
  const response = {};

  if (upvotedComment && database.users[username]) {
    upvotedComment = upvote(upvotedComment, username);
...
function downvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let downvotedComment = database.comments[id];
  const response = {};

  if (downvotedComment && database.users[username]) {
    downvotedComment = downvote(downvotedComment, username);
...
function loadDatabase() {
  ...
  if (fs.existsSync('database.yaml')) {
    database = fs.readFileSync('database.yaml');
    database = yaml.safeLoad(database);
  } else {
    database = {
      users: {},
      articles: {},
